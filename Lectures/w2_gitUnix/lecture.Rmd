...
This section will focus on how to install things (and permissions, generally) on Ubuntu. If we want to install a program called htop, we could issue:
...
We may want to multiple tasks that build on one another (e.g., sort the output of ls). To do this, a useful method is to use the unix pipe. The pipe basically takes the output of the first thing and hands it to the next thing, separated by a "|" character. For instance, if we want to list the contents of the working directory, and then sort them, we could issue
